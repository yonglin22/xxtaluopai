...
### 愈池 v4 · 玩法补全（本轮）
- **今日缘分**（`#fate`，`/api/fate`）：站内匿名加好友（雅号不暴露手机号）+ **24h 阅后即焚**私信 + 反站外导流。
- **精选诗集**（`#anthology`）：10 首精选诗 + 池水「被点亮最多的心事」可翻阅。
- **作品集 / 年度合集**（情绪架内，`/api/shelf` 返回 myPoems）：汇总我投进池子的诗，一键生成**年度情绪合集海报**分享。
- **长期养成**：连续投递 streak + 里程碑（1/7/30/100/365，达成弹祝贺），情绪罐展示连续天数。
- **PUGC 引导模板**：发布器「借句诗开头」，帮没文采的用户起笔。
- **每日随机背景音乐**：按当天轮换 5 首柔和睡前曲（程序合成，可替换为真实音频文件）。
- **问一事两步向导**：设定（缩小卡扇 banner + 面/记录/牌阵/起牌师）→ 提问（摘要·起牌师可切换 + 问题框 + 常见问题 + 开始洗牌），同屏滑动切换、状态保活、修复 iOS 键盘聚焦。
- **两处海报**（问一事 banner / 愈池池水）统一为**毛玻璃白 + 暗金呼吸光晕**，对齐首页。
> 待素材：3D 商品图（需真实 3D 渲染/素材）另行处理。
